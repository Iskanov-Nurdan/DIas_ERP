...
Каждый запрос получает сквозной **`X-Request-Id`**: nginx его генерирует (или берёт из заголовка клиента),
передаёт в Django, возвращает фронту в заголовке ответа и пишет в оба лога. По нему связываются все звенья.

### 1. Взять id из ответа

В DevTools браузера (вкладка Network → Headers) или через curl:

```bash
curl -sD- -o /dev/null http://5.42.98.29/api/me/ | grep -i x-request-id
```

При 502 id приходит прямо в теле ответа:
`{"code":"backend_unavailable","error":"Сервис временно недоступен","request_id":"a1b2c3..."}`

### 2. Посмотреть весь путь запроса

```bash
make trace RID=a1b2c3...
```

Выведет строку nginx и все строки Django с трассировкой:

```
── nginx ──
172.18.0.1 [23/Aug/2026:17:41:57] "POST /api/sales/ HTTP/1.1" status=500 upstream=500
    bytes=144 rt=0.312 urt=0.310 rid=a1b2c3... ref="-" ua="Mozilla/5.0"
── backend ──
2026-08-23 20:41:57 [ERROR] config.exceptions request_id=a1b2c3...
    Необработанное исключение [IntegrityError]: duplicate key value violates unique constraint
    — POST /api/sales/ | user=admin (id=2) | rid=a1b2c3... | view=SaleViewSet
Traceback (most recent call last): ...
```

Строка сразу отвечает на три вопроса: **что** (`IntegrityError` + текст БД), **где** (`POST /api/sales/`,
`view=SaleViewSet`, файл и номер строки в трассировке), **у кого** (`user=admin (id=2)`).

### 3. Команды по ситуации

| Команда              | Когда                                                        |
|----------------------|--------------------------------------------------------------|
| `make doctor`        | Первое, что запускать: статус контейнеров, health, свежие ошибки, место на диске |
| `make errors`        | Только `[ERROR]` и трассировки Django за сутки               |
| `make http-errors`   | Все 4xx/5xx из access-лога nginx — видно, что именно падает   |
| `make trace RID=...` | Полный путь одного запроса                                    |
| `make logs-backend`  | Живой хвост логов Django                                      |
| `make logs-nginx`    | Живой хвост nginx                                             |
| `make logs-db`       | Логи PostgreSQL (дедлоки, обрывы соединений)                  |
| `make conf-check`    | `nginx -t` + `manage.py check --deploy`                       |

### 4. Как читать строку nginx

```
status=502 upstream=504 rt=3.072 urt=3.070
```

- `status` — что увидел клиент, `upstream` — что ответил Django.
- `status=502`, `upstream=-` или `504` — бэкенд не ответил вовсе: смотреть `make logs-backend`.
- `status=500`, `upstream=500` — исключение в Django: `make errors`, трассировка на месте.
- `status=4xx` — ошибка запроса (нет токена, нет прав, невалидные данные); Django пишет её `[WARNING]`
  с указанием пользователя и вью.
- `rt` заметно больше `urt` — время потерялось на сети/клиенте, а не в бэкенде.

### 5. Что логируется само

- **5xx** — `[ERROR]` с типом исключения, полной трассировкой, методом, путём, пользователем и request_id.
- **4xx** — `[WARNING]` с тем же контекстом (кто, куда, почему отказ).
- **Ошибки старта** (нет `SECRET_KEY`, недоступна БД, битая миграция) — в логе контейнера, статус `unhealthy` в `make ps`.
- **Падение бэкенда** — клиент получает JSON `backend_unavailable` (не HTML-заглушку nginx) за ~3 секунды,
  контейнер перезапускается сам (`restart: unless-stopped`).

Клиенту при 500 отдаётся только «Внутренняя ошибка сервера» — трассировки и SQL наружу не утекают,
они остаются в логах контейнера.

### 6. Больше подробностей

```bash
# в .env.prod
LOG_LEVEL=DEBUG
```
затем `make restart`. Включает подробные логи проекта **без** `DEBUG=True` — то есть без риска
показать трассировку пользователю. После разбора верните `INFO`.

Никогда не ставьте `DEBUG=True` на боевом сервере: Django начнёт отдавать трассировки со значениями
переменных окружения любому, кто откроет страницу.
