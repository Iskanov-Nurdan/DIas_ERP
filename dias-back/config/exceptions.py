...
def _make_error_response(code: str, message: str, errors: list = None, http_status: int = 400) -> Response:
    payload = {
        'error': {
            'code': code,
            'message': message,
...
def dias_exception_handler(exc, context):
    ...
    if isinstance(exc, Throttled):
        ...
        return _make_error_response('too_many_requests', msg, http_status=429)
    ...
    if isinstance(detail, dict):
        message = str(detail.get('detail', '') or detail.get('error', '') or _DEFAULT_MESSAGES.get(http_status, 'Ошибка'))
